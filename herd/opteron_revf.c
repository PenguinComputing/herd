...
int orevf_translate(decode_input_t* dinput, decode_result_t* result)
{
    u32 SystemAddr = (u32)(dinput->address >> 8);

    int CSFound, NodeID, CS = 0, F1Offset, F2Offset, F2MaskOffset, Ilog;
    ...
    if (pci_read_config(0, 0x18 + NodeID, 2, 0x90) & (1 << 11)) {
	int symbol, bits;
...
void orevf_init_cpu(Cpu* cpu)
{
#ifdef WIN32
	wOptions.CpuDecode = cpu;
#endif /* WIN32 */
    cpu->show_config = orevf_show_config;
    cpu->translate = orevf_translate;
}
